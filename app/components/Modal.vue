<template>
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 backdrop-blur-sm p-4" @click.self="closeOnClickOutside && emit('update:modelValue', false)">
        <div :class="['bg-white/90 backdrop-blur-xl rounded-2xl border border-white shadow-2xl shadow-cyan-900/10 w-full flex flex-col', sizeMap[size]]" @click.stop>
          <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <button class="w-8 h-8 rounded-full hover:bg-slate-100 flex items-center justify-center transition-colors text-slate-400 hover:text-slate-600" @click="closeOnClickOutside && emit('update:modelValue', false)">
